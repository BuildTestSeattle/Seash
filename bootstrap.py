...
copies the necessary files to run Seash into it.
...
<Usage>
  bootstrap.py [-t] [-v] [-c] [-r] <target_directory>
...
<Example>
  ...
  and run the unit tests for module "repyv2api" there.
    ...
    user@vm:dist$ python bootstrap.py -t /tmp/test
...
def main():
  ...
  helpstring = "python bootstrap.py [-t] [-v] [-c] [-r] <target>"
  ...
  copy_tree_to_target("seash/", os.path.join(target_dir, 'seash/'), ignore=".git")
  copy_tree_to_target("seattlelib_v2/", os.path.join(target_dir, 'seattlelib_v2/'), ignore=".git")
  copy_tree_to_target("clearinghouse/", os.path.join(target_dir, 'clearinghouse/'), ignore=".git")
  copy_tree_to_target("portability/", os.path.join(target_dir, 'portability/'), ignore=".git")
  copy_tree_to_target("repy_v2/", os.path.join(target_dir, 'repy_v2/'), ignore=".git")
  copy_tree_to_target("repy_v1/", os.path.join(target_dir, 'repy_v1/'), ignore=".git")
  ...
  if repytest:
    ...
    copy_to_target("seattlelib_v2/testlibrepy/*", target_dir)
    copy_to_target("seattlelib_v2/test/*", target_dir)
    copy_to_target("clearinghouse/tests/*", target_dir)
